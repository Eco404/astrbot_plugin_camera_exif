...
| 指令 | 说明 |
|------|------|
| `/exif help` | 显示帮助菜单和可查询字段 |
| `/exif full` | 查询完整 EXIF 元数据 |
| `/exif get <字段>` | 查询指定字段，例如 `/exif get 相机型号` |

`get` 支持原字段名及其常用中英文别名，例如 `快门次数`、`SC`、`ISO`、`model`、`lens`、`resolution`、`GPS`。

### 使用方式
1. `[图片] /exif full` — 发送图片的同时查询完整 EXIF
2. `[引用图片] /exif get ISO` — 查询已发送图片的指定字段
3. `/exif get 焦距` → 等待图片 — 先输入指令，再发图片（超时 120s）
